...
class QuadrotorMpcTrajectory(Thread):
    def __init__(self, name: str, trajectory_type: str, generate_c_code, quadrotor: Quadrotor, horizon: float, num_steps: int,  solution_callback, x0, xr):
        super(QuadrotorMpcTrajectory, self).__init__(target=self.solve_mpc_control, args=(x0, xr))
    ...
    def generate_mpc(self):
        ...
        ocp.code_export_directory = self.acados_generated_files_path / 'quadrotor_c_generated_code'
        ...
        if self.trajectory_type == 'circle':      
            a = 1.0
            omega = 1.5*tanh(0.1*t)
            initial_position = [0., 0., 1.0]
            pxr = initial_position[0] + a*cos(omega*t)
            pyr = initial_position[1] + a*sin(omega*t)
            pzr = initial_position[2]
            vxr = -a*omega*sin(omega*t)
            vyr = a*omega*cos(omega*t)
            vzr = 0.0
